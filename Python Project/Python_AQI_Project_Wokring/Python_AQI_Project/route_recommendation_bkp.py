...
for location, path in files.items():
    ...
    output_path = os.path.join("maps", "recommended_all_routes_map.html") '''
...
def suggest_cleanest_nearby_location(user_lat, user_lon, trained_models, original_dfs,output_path):
...
def run_route_recommendation():
    ...
    for location, path in files.items():
        model_path = f"saved_models/{location}_model.joblib"
        if not os.path.exists(path) or not os.path.exists(model_path):
            print(f"⚠️ Skipping {location} (missing data or model)")
            continue
    ...
    # --- Get User Input and Suggest Route ---
    try:
        ...
        print("❌ Invalid input for latitude/longitude.")
